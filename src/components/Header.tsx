              ...
              <div className="absolute right-0 mt-2 w-72 bg-white dark:bg-slate-900 rounded-2xl shadow-xl border border-slate-200 dark:border-slate-800 py-2 z-50 animate-in fade-in zoom-in-95 duration-150">
                ...
                {onResetBaselineData && (
                  <button
                    ...
                    className="w-full text-left px-3.5 py-2 text-xs font-bold text-amber-900 dark:text-amber-300 hover:bg-amber-50/80 dark:hover:bg-amber-950/40 flex items-center gap-2.5 transition-all cursor-pointer"
                  >
                    <div className="w-7 h-7 bg-amber-100 dark:bg-amber-900/60 text-amber-700 dark:text-amber-300 rounded-lg flex items-center justify-center shrink-0">
                      <RotateCcw className="w-3.5 h-3.5" />
                    </div>
                    <div>
                      <span className="block font-black">{language === 'es' ? 'Restablecer Datos de Muestra' : 'Reset Sample Baseline Data'}</span>
                      <span className="text-[10px] text-amber-700 dark:text-amber-400 font-normal block">{language === 'es' ? 'Restaurar registros iniciales de demostración' : 'Restore initial sample demo records'}</span>
                    </div>
                  </button>
                )}
                ...
                {onOpenGmailSync && (
                  ...
                  >
                    ...
                    <div>
                      <span className="block font-black">{language === 'es' ? 'Sincronizar Pagos Gmail' : 'Sync Gmail Payments'}</span>
                      <span className="text-[10px] text-rose-600 dark:text-rose-400 font-normal block">Extract Venmo/Cash App emails</span>
                    </div>
                  </button>
                )}
                ...
                {onOpenExcludedPerformers && (
                  <button
                    ...
                    className="w-full text-left px-3.5 py-2 text-xs font-bold text-rose-900 dark:text-rose-300 hover:bg-rose-50 dark:hover:bg-rose-950/40 flex items-center gap-2.5 transition-all cursor-pointer border-t border-slate-100 dark:border-slate-800"
                  >
                    <div className="w-7 h-7 bg-rose-100 dark:bg-rose-900/60 text-rose-700 dark:text-rose-300 rounded-lg flex items-center justify-center shrink-0">
                    ...
                    <div>
                      ...
                      <span className="text-[10px] text-rose-600 dark:text-rose-400 font-normal block">EXCLUDED_PERFORMERS system accounts</span>
                    </div>
                  </button>
                )}
                ...
                >
                  ...
                  <div>
                    <span className="block font-black">{language === 'es' ? 'Diagnóstico Code.gs' : 'Code.gs Diagnostics'}</span>
                    <span className="text-[10px] text-slate-500 dark:text-slate-400 font-normal block">{language === 'es' ? 'Pruebas y rendimiento del script' : 'Engine benchmark & execution tests'}</span>
                  </div>
                </button>
                ...
                {/* Widgets & Popups Sub-Header */}
                <div className="px-3 py-1.5 border-t border-b border-slate-100 dark:border-slate-800 my-1 bg-slate-50 dark:bg-slate-800/50">
                  <p className="text-[10px] font-extrabold uppercase text-indigo-600 dark:text-indigo-400 tracking-wider flex items-center gap-1">
                    <BarChart3 className="w-3 h-3" />
                    <span>{language === 'es' ? 'Widgets y Métricas (Popups)' : 'Widgets & Analytics Popups'}</span>
                  </p>
                </div>
                ...
                <button
                  onClick={() => {
                    setIsDataDropdownOpen(false);
                    if (onOpenWidgetModal) onOpenWidgetModal('financialRuleset');
                  }}
                ...
                >
                  ...
                  <div>
                    <span className="block font-black text-[11px]">{language === 'es' ? 'Reglas Financieras' : 'Financial Ruleset'}</span>
                    <span className="text-[9px] text-slate-500 dark:text-slate-400 font-normal block">Dues rates, grace period & fees</span>
                  </div>
                </button>
                ...
                <button
                  onClick={() => {
                    setIsDataDropdownOpen(false);
                    if (onOpenWidgetModal) onOpenWidgetModal('executiveSummary');
                  }}
                  className="w-full text-left px-3.5 py-1.5 text-xs font-bold text-slate-800 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 flex items-center gap-2.5 transition-all cursor-pointer"
                >
                  <div className="w-6 h-6 bg-indigo-100 dark:bg-indigo-900/60 text-indigo-700 dark:text-indigo-300 rounded-lg flex items-center justify-center shrink-0">
                  ...
                  <div>
                    <span className="block font-black text-[11px]">{language === 'es' ? 'Resumen Ejecutivo' : 'Executive Dues Summary'}</span>
                    <span className="text-[9px] text-slate-500 dark:text-slate-400 font-normal block">YTD revenue & collection rate</span>
                  </div>
                </button>
                ...
                <button
                  onClick={() => {
                    setIsDataDropdownOpen(false);
                    if (onOpenWidgetModal) onOpenWidgetModal('volumeTrend');
                  }}
                  className="w-full text-left px-3.5 py-1.5 text-xs font-bold text-slate-800 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 flex items-center gap-2.5 transition-all cursor-pointer"
                >
                  <div className="w-6 h-6 bg-indigo-100 dark:bg-indigo-900/60 text-indigo-700 dark:text-indigo-300 rounded-lg flex items-center justify-center shrink-0">
                  ...
                  <div>
                    <span className="block font-black text-[11px]">{language === 'es' ? 'Volumen Total 30 Días' : '30-Day Payment Volume Trend'}</span>
                    <span className="text-[9px] text-slate-500 dark:text-slate-400 font-normal block">Daily & cumulative volume chart</span>
                  </div>
                </button>
                ...
                <button
                  onClick={() => {
                    setIsDataDropdownOpen(false);
                    if (onOpenWidgetModal) onOpenWidgetModal('channelBreakdown');
                  }}
                  className="w-full text-left px-3.5 py-1.5 text-xs font-bold text-slate-800 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 flex items-center gap-2.5 transition-all cursor-pointer"
                >
                  <div className="w-6 h-6 bg-indigo-100 dark:bg-indigo-900/60 text-indigo-700 dark:text-indigo-300 rounded-lg flex items-center justify-center shrink-0">
                  ...
                  <div>
                    <span className="block font-black text-[11px]">{language === 'es' ? 'Desglose por Canal' : 'Payment Channel Intake Breakdown'}</span>
                    <span className="text-[9px] text-slate-500 dark:text-slate-400 font-normal block">Venmo, Zelle, Cash App, Direct & Cash</span>
                  </div>
                </button>
                ...
                <button
                  onClick={() => {
                    setIsDataDropdownOpen(false);
                    if (onOpenWidgetModal) onOpenWidgetModal('modularArchitecture');
                  }}
                  className="w-full text-left px-3.5 py-1.5 text-xs font-bold text-slate-800 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-800 flex items-center gap-2.5 transition-all cursor-pointer"
                >
                  <div className="w-6 h-6 bg-indigo-100 dark:bg-indigo-900/60 text-indigo-700 dark:text-indigo-300 rounded-lg flex items-center justify-center shrink-0">
                  ...
                  <div>
                    <span className="block font-black text-[11px]">{language === 'es' ? 'Arquitectura Modular' : 'Modular Architecture'}</span>
                    <span className="text-[9px] text-slate-500 dark:text-slate-400 font-normal block">In-Memory Engine & GAS Sync</span>
                  </div>
                </button>
